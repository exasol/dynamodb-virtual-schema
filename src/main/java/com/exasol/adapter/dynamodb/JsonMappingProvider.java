...
import com.exasol.adapter.dynamodb.mapping_definition.StringColumnMappingDefinition;
...
import com.exasol.adapter.dynamodb.mapping_definition.result_walker.DynamodbResultWalkerBuilder;
...
public class JsonMappingProvider implements MappingProvider {
    ...
    private void addRootDefinition(final JSONObject definition, final String fileName) throws MappingException {
        final TableMappingDefinition.Builder tableBuilder = TableMappingDefinition.builder(definition.getString(DEST_TABLE_NAME_KEY),true);
        walkRootMapping(definition.getJSONObject(MAPPING_KEY), new IdentityDynamodbResultWalker.Builder(), tableBuilder);

        this.tables.add(tableBuilder.build());
    }


    private void walkRootMapping(final JSONObject definition, final DynamodbResultWalkerBuilder walkerToThisPath, final TableMappingDefinition.Builder tableBuilder) throws MappingException {
        walkMapping(definition, walkerToThisPath, tableBuilder, null, true);
    }

    private void walkMapping(final JSONObject definition, final DynamodbResultWalkerBuilder walkerToThisPath, final TableMappingDefinition.Builder tableBuilder, final String propertyName, final boolean isRootLevel) throws MappingException {
        if(definition.has(TO_STRING_MAPPING_KEY)) {
            if (isRootLevel) {
                throw new MappingException("ToString mapping is not allowed at root level");
            }
            addStringColumn(definition.getJSONObject(TO_STRING_MAPPING_KEY), walkerToThisPath, tableBuilder, propertyName);
        }
        else if(definition.has(TO_JSON_MAPPING_KEY)) {
            addToJsonColumn(definition.getJSONObject(TO_JSON_MAPPING_KEY), walkerToThisPath, tableBuilder, propertyName);
        }
    ...
    private void walkChildren(final JSONObject definition, final DynamodbResultWalkerBuilder walkerToThisPath, final TableMappingDefinition.Builder tableBuilder ) throws MappingException {
        for (final String dynamodbPropertyName : definition.keySet()) {
            final ObjectDynamodbResultWalker.Builder walker = new ObjectDynamodbResultWalker.Builder(walkerToThisPath, DynamodbResultWalker.LookupFailBehaviour.EXCEPTION, dynamodbPropertyName);//TODO: add lookup fail behaviour to language
            this.walkMapping(definition.getJSONObject(dynamodbPropertyName),walker, tableBuilder, dynamodbPropertyName, false);
        }

    }

    private void addStringColumn(final JSONObject definition, final DynamodbResultWalkerBuilder resultWalker, final TableMappingDefinition.Builder tableBuilder, final String dynamodbPropertyName){
        ...
        tableBuilder.withColumnMappingDefinition(new StringColumnMappingDefinition(destinationColumnName, maxLength, resultWalker.build(), overflowBehaviour));
    }

    private void addToJsonColumn(final JSONObject definition, final DynamodbResultWalkerBuilder resultWalker, final TableMappingDefinition.Builder tableBuilder, final String dynamodbPropertyName) throws MappingException {
        String destinationColumnName =  dynamodbPropertyName;
        if(definition.has(DEST_NAME_KEY)){
            destinationColumnName = definition.getString(DEST_NAME_KEY);
        }
        ...
        tableBuilder.withColumnMappingDefinition(new ToJsonColumnMappingDefinition(destinationColumnName, resultWalker.build()));
    }
