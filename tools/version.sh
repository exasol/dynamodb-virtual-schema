#!/bin/bash
readonly vs_jar_prefix='document-virtual-schema-dist'
...
prepare() {
    ...
    readonly common_document_version=$(extract_common_document_version "$master_pom")
    log "Found virtual-schema-common-java version $common_document_version in master file \"$master_pom\""
...
extract_common_document_version() {
    grep -oP "<vs-common-document.version>[^<]*<" "$1" | head -n1 | sed -e's/^.*>\s*//' -e's/\s*<//'
}
...
update_documentation() {
    ...
    find -type f -regextype posix-extended -regex "$file_find_regex" -not -path "./doc/changes/*" \
      ...
      -exec sed -i s/"$vs_jar_pattern"/"$vs_jar_prefix-$common_document_version-dynamodb-$dialects_version.$jar_suffix"/g {} \;
}
