...
1. Upload the adapter to the BucketFS:
    ``` shell script
   curl -I -X PUT -T document-virtual-schema-dist-0.3.0-dynamodb-0.5.0.jar
   ```
...
1. Create the Adapter Script:
    ...
    CREATE OR REPLACE JAVA ADAPTER SCRIPT ADAPTER.DYNAMODB_ADAPTER AS
       ...
       %jar /buckets/bfsdefault/default/document-virtual-schema-dist-0.3.0-dynamodb-0.5.0.jar;
    /
    ```
1. Create UDF:
    ```sql
