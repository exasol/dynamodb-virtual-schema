...
1. Upload the adapter to the BucketFS:
   ```shell
   curl -I -X PUT -T document-virtual-schema-dist-12.0.2-dynamodb-4.0.1.jar http://w:writepw@<YOUR_DB_IP>:2580/default/
   ```
...
1. Create the Adapter Script:
    ...
    CREATE OR REPLACE JAVA ADAPTER SCRIPT ADAPTER.DYNAMODB_ADAPTER AS
       ...
       %jar /buckets/bfsdefault/default/document-virtual-schema-dist-12.0.2-dynamodb-4.0.1.jar;
    /
    ```
1. Create UDF:
    ```sql
